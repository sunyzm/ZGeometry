...
namespace ZGeom
{
    ...
    template<typename T>
    inline void 
        SparseMatrix<T>::toIdentity(uint order)
    {
    ...
    template<typename T>
    inline void
        SparseMatrix<T>::read(std::istream& in)
    {
    ...
    template<typename T> 
    inline bool 
        SparseMatrix<T>::testNoEmptyRow() const
    {
    ...
    template<typename T>
    inline bool 
        SparseMatrix<T>::testSymmetric() const
    {
        for (uint k = 0; k < mNonzeroCount; ++k) {
    ...
    template<typename T> 
    inline void 
        SparseMatrix<T>::fillEmptyDiagonal()
    {
    ...
    template<typename T> 
    inline void 
        SparseMatrix<T>::truncate(MatrixForm form /*=MAT_UPPER*/)
    {
        assert(mRowCount == mColCount);
