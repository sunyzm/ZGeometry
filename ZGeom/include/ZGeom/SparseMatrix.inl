...
{
    std::function<bool(const MatElem<T>&, const MatElem<T>&)> funcLess = [](const MatElem<T>& e1, const MatElem<T>& e2) {
        if (e1.row < e2.row || (e1.row == e2.row && e1.col < e2.col)) return true;
        else return false;
    };
    std::sort(mElements.begin(), mElements.end(), funcLess);
}
