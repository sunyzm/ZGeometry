...
void ShapeEditor::testArtificialShapeMCA()
{
    ...
    for (int i = 0; i < totalVertCount; ++i) {
        vColors0[i] = ColorMap::falseColor((vX0[i] - x_min) / (x_max - x_min), 1.f, ZGeom::CM_JET);
    }
    ...
    for (int i = 0; i < totalVertCount; ++i) {
        vColors1[i] = ColorMap::falseColor((vX1[i] - x_min) / (x_max - x_min), 1.f, ZGeom::CM_JET);
    }
    ...
    for (int i = 0; i < totalVertCount; ++i) {
        vColors2[i] = ColorMap::falseColor(min(float(vDiff[i] / (bbDiag*0.02)), 1.f), 1.f, ZGeom::CM_COOL);
    }
	...
	for (int i = 0; i < totalVertCount; ++i) {
		vColorsMCA1[i] = ColorMap::falseColor((vXC[i] - x_min) / (x_max - x_min), 1.f, ZGeom::CM_JET);
		vColorsMCA2[i] = ColorMap::falseColor(min(float(fabs(vXT[i] / (0.02*bbDiag))), 1.f), 1.f, ZGeom::CM_COOL);
	}
...
void ShapeEditor::testDenoisingDLRS()
{
    ...
    for (int i = 0; i < totalVertCount; ++i) {
        vColorResApprox[i] = ColorMap::falseColor(fabs(vResApprox[i] / (0.05*bbDiag)));
    }
...
void ShapeEditor::holeEstimateCurvature()
{
    ...
    for (int i = 0; i < totalVertCount; ++i) {
        colorOldCurv[i] = ColorMap::falseColor((oldCurvature[i] - min_curv) / (max_curv - min_curv));
        colorNewCurv[i] = ColorMap::falseColor((newCurvature[i] - min_curv) / (max_curv - min_curv));
    }
